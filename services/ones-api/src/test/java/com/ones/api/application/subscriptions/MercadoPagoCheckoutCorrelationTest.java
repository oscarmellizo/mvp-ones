...
class MercadoPagoCheckoutCorrelationTest {
    ...
    void createsCheckoutPlanWithUserReferenceForHostedCheckout() {
        ...
        when(mercadoPago.createPlan(
                any(), any(), any(Long.class), any(), any(), any(), any()
        )).thenReturn(new MercadoPagoGateway.PreapprovalPlan(
                "checkout-plan-123",
                "Ones Plus Monthly",
                "https://www.mercadopago.com.co/subscriptions/checkout?preapproval_plan_id=checkout-plan-123",
                "user-123"
        ));
        ...
        verify(mercadoPago).createPlan(
                eq("Ones Plus Monthly"),
                eq("month"),
                eq(19900L),
                eq("COP"),
                eq("https://app.ones.events/plans/success?ones_uid=user-123"),
                eq("https://app.ones.events/v1/payments/mercadopago/webhook"),
                eq("user-123")
        );
